...
{
    std::mutex VMapMutex;
    std::atomic<VMAP::IVMapManager*> VMapManager { nullptr };
}
...
namespace VMAP
{
    ...
    {
        IVMapManager* instance = VMapManager.load(std::memory_order_acquire);
        if (!instance)
        {
            std::lock_guard<std::mutex> lock(VMapMutex);
            instance = VMapManager.load(std::memory_order_relaxed);
            if (!instance)
            {
                instance = new VMapManager2();
                VMapManager.store(instance, std::memory_order_release);
            }
        }
        return *instance;
    }
