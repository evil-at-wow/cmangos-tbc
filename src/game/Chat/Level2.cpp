...
bool ChatHandler::HandleMmapPathCommand(char* args)
{
    ...
    {
        if (!MMAP::MMapFactory::getMMapManager().GetGONavMesh(transport->GetDisplayId()))
        {
    ...
    {
        if (!MMAP::MMapFactory::getMMapManager().GetNavMesh(m_session->GetPlayer()->GetMapId()))
        {
            PSendSysMessage("NavMesh not loaded for current map.");
            return true;
        }
...
bool ChatHandler::HandleMmapLocCommand(char* /*args*/)
{
    ...
    const dtNavMesh* navmesh = MMAP::MMapFactory::getMMapManager().GetNavMesh(player->GetMapId());
    const dtNavMeshQuery* navmeshquery = MMAP::MMapFactory::getMMapManager().GetNavMeshQuery(player->GetMapId(), player->GetInstanceId());
...
bool ChatHandler::HandleMmapLoadedTilesCommand(char* /*args*/)
{
    ...
    const dtNavMesh* navmesh = MMAP::MMapFactory::getMMapManager().GetNavMesh(mapid);
    const dtNavMeshQuery* navmeshquery = MMAP::MMapFactory::getMMapManager().GetNavMeshQuery(mapid, m_session->GetPlayer()->GetInstanceId());
    if (!navmesh || !navmeshquery)
    {
        PSendSysMessage("NavMesh not loaded for current map.");
        return true;
...
bool ChatHandler::HandleMmapStatsCommand(char* /*args*/)
{
    ...
    MMAP::MMapManager& manager = MMAP::MMapFactory::getMMapManager();
